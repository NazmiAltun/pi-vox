...
import voiceInputExtension, { matchesVoiceShortcut, shouldScheduleSpacePress } from '../extensions/pi-vox.js';
...
test('voice extension installs an editor component when Pi CustomEditor dependency resolves', async () => {
  ...
  assert.ok(notifications.some((entry) => entry.includes('voice ready')));
});
